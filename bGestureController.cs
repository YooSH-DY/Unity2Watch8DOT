...
public class GestureController : MonoBehaviour
{
    ...
    private const float MAX_ROLL_SUM = 170f;     // 최대 롤 합계 (절대값)
    ...
    private void CheckKeyboardInput()
    {
        ...
        {
            // 백워킹 중에는 Q키만 특별히 허용
            if (!(currentAnimationName == "BackWalking" && Input.GetKeyDown(KeyCode.Q)))
            {
                return;
            }
        }
        ...
        else if (Input.GetKeyDown(KeyCode.W)) 
        {
            ...
            // 점프 상태 설정 (중요: 걷기 상태에서 시작)
            jumpState = JumpState.WaitingForDown;
            jumpBase = true;
            jumpReady = false;
            jumping = false;
            
            // Roll 관련 변수 초기화 (중요: minRoll을 높은 값으로 설정)
            currentRoll = 0f;
            minRoll = 0f;  
            maxRoll = 0f; 
            isWaitingForRollUp = false;
            
            // 걷기 애니메이션으로 시작 (Roll이 내려갈 때까지)
            animator.CrossFade("Walk", 0.2f);
            
            // 이동 상태 활성화 - Roll이 내려갈 때까지 계속 이동
            if (playerController != null)
                playerController.isMove = true;
            
            Debug.Log("점프 초기화 완료: Roll 값이 내려가길 대기 중...");
            StartCoroutine(ImprovedJump());
        }
    ...
    {
        // jumpBase 상태 처리 (걷기 상태)
        if(jumpBase){
            animator.CrossFade("Walk", 0.2f);
            jumpState = JumpState.WaitingForDown;
            
            // 이동 활성화 (Roll이 내려갈 때까지)
            if (playerController != null)
                playerController.isMove = true;
                
            Debug.Log("점프 준비 시작: 걷기 중 Roll 각도 감소 대기");
            yield break; // 여기서 종료, UpdateRollData에서 Roll 감지 시 다시 호출
        }
        
        // jumpReady 상태 처리 부분 수정
        if(jumpReady){
            // 원래 위치 저장 (중요)
            Vector3 originalPosition = transform.position;
            
            animator.CrossFade("Jump", 0.32f);
            animator.SetFloat("JumpPhase", 0);
            ...
            // Jump 애니메이션 1단계 (웅크림)로 설정
            animator.Play("Jump", 0, 0);
            animator.SetFloat("JumpPhase", 1);
            jumpState = JumpState.WaitingForUp;
            readyAnimStartTime = Time.time;
            
            // 위치 복원 - Y축 값이 애니메이션에 의해 변경되었다면 원래대로 복원
            ...
            if (currentPosition.y < originalPosition.y)
            {
                ...
                Debug.Log($"웅크림 시 위치 보정: {currentPosition.y:F3} → {originalPosition.y:F3}");
            }
            
            // Roll 각도에 따른 웅크림 깊이 조절
            float negativeRoll = Mathf.Abs(minRoll);
            float clampedRoll = Mathf.Clamp(negativeRoll, 10f, 80f); 
            float squatIntensity = Mathf.InverseLerp(10f, 80f, clampedRoll);
            
            // 웅크림 강도에 따라 애니메이션 속도 조절
            animator.speed = Mathf.Lerp(1.0f, 0.7f, squatIntensity);
            
            Debug.Log($"웅크림 시작: Roll={minRoll:F1}°, 강도={squatIntensity:F2}, 속도={animator.speed:F2}");
            yield break; // 여기서 종료, UpdateRollData에서 Roll 상승 감지 시 다시 호출
        }
        ...
        if(jumping){
            ...
            // 도약 모션 시간 대기 (backupGestureController에서 가져온 시간)
            yield return new WaitForSeconds(0.2f);
            
            // 롤 각도 값에 따른 점프 높이 계산
            float rollSum = Mathf.Abs(minRoll) + Mathf.Abs(maxRoll);
            Debug.Log($"점프 높이 계산: 롤 합계={rollSum:F1}° (최소={minRoll:F1}°, 최대={maxRoll:F1}°)");
            
            // 롤 각도 합에 따른 점프 높이 계산 - 구간 내에서 비례 계산 추가
            float jumpHeight;
            
            // 단계별 임계값과 높이 정의 (각 구간의 시작점)
            float[] rollThresholds = { 20f, 50f, 80f, 110f, 140f, 170f };
            ...
            //Debug.Log($"롤 각도 합({rollSum:F2})에 따른 최종 점프 높이: {jumpHeight:F2}m");
            ...
            if (playerController != null && playerController.rb != null)
            {
                ...
                //Debug.Log($"물리 점프 적용: 높이 {jumpHeight:F2}m, 롤 합계 {rollSum:F2}°");
            }
            
            // JumpAir 상태로 전환 (backupGestureController에서 가져온 시간)
            yield return new WaitForSeconds(0.35f);
            animator.SetFloat("JumpPhase", 3);
            jumpState = JumpState.JumpAir;
            Debug.Log("개선된 점프: JumpAir 상태 전환");

            // JumpAir 상태에서 Y축 값을 체크 - backupGestureController와 동일한 로직
            bool hasReachedPeak = false;
            bool hasTransitionedToDown = false;
            float checkStartTime = Time.time;
            float previousHeight = transform.position.y;
            ...
            // 최대 시간 제한 추가 (backupGestureController에서 가져옴)
            while (Time.time - checkStartTime < 5.0f && !hasTransitionedToDown)
            {
                ...
                if (hasReachedPeak && currentHeight <= 2.0f)
                {
                    ...
                    animator.Play("Jump", 0, 0);
                    animator.SetFloat("JumpPhase", 4);
                    animator.speed = 1.05f; // 백업 스크립트에서의 속도 값 적용
                    jumpState = JumpState.JumpingDown;
            ...
            // 만약 높이 체크에서 전환되지 않았다면 여기서 강제로 전환 (백업 스크립트에서 가져옴)
            ...
            while (Time.time - landingStartTime < 1.0f && !hasLanded)
            {
                ...
                {
                    hasLanded = true;
                    //Debug.Log($"착지 감지! 높이: {currentHeight:F2}m, IsGrounded: {isGrounded}");
                    break;
                }
            ...
            if (hasLanded)
            {
                float smoothTime = 0.15f; // 백업 스크립트의 보간 시간
                float elapsedTime = 0f;
                Vector3 startPos = transform.position;
                Vector3 targetPos = new Vector3(startPos.x, 0f, startPos.z);
    ...
    private System.Collections.IEnumerator MaintainBackwalkingState()
    {
        ...
        {
            // 직선 이동 상태일 때 일정 시간 후 원형 이동으로 전환
            if (playerController.currentState == PlayerController.MovementState.Straight)
            {
                // 일정 시간마다 원형 이동으로 전환 시도
                playerController.TryCircleMovement();
            }
    ...
    // 애니메이션 잠금 관리 (기존 코드 유지)
    private void UpdateAnimationLock()
    {
        // BackWalking 애니메이션은 명시적인 트리거가 있을 때까지 계속 유지
    ...
    private void HandleHandGesture(int fingerCount, int palmOrientation)
    {
        ...
        {
            Debug.Log($"제스처 무시됨: 애니메이션 잠금 상태 (isAnimationLocked={isAnimationLocked})");
            return;
        }
        ...
        switch (fingerCount)
        {
            ...
            case 2: // 점프
                ...
                Debug.Log("손가락 2개 제스처: W 키와 동일하게 점프 시작");
                StartCoroutine(ImprovedJump());
                break;
            case 1: // 뒤로 걷기 - 수정
                StopAllCoroutines();
    ...
    {
         Debug.Log($"OnAnimatorIK 호출: jumpState={jumpState}, isHuman={animator?.isHuman}, layerIndex={layerIndex}");
        ...
        // GyroY 값에 따른 웅크림 강도 계산
        float negativeGyroY = Mathf.Abs(lowestGyroY);
        
        // 웅크림 깊이를 더 제한된 범위로 조정 (60~150으로 제한)
        float clampedGyroY = Mathf.Clamp(negativeGyroY, 60f, 300f);
        float squatIntensity = Mathf.InverseLerp(60f, 300f, clampedGyroY);
        ...
        if (animator.isHuman)
        {
            // 기본 웅크림 깊이: 캐릭터 키의 약 25%로 제한
            float characterHeight = 1.7f; // 일반적인 캐릭터 키 (미터)
            float maxSquatDepth = characterHeight * 0.4f; 
    ...
    void OnGUI()
    {
        if (jumpState == JumpState.WaitingForDown || jumpState == JumpState.WaitingForUp)
        {
            ...
            if (isWaitingForRollUp)
                GUI.Label(new Rect(10, 30, 300, 20), "웅크림 감지됨! 위로 올리세요");
        }
    }
}
